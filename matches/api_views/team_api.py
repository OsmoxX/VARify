...
def get_team_detail(request, api_id):
    ...
    try:
        ...
        return Response(
            {"detail": "Drużyna nie znaleziona."}, status=status.HTTP_404_NOT_FOUND
        )
...
def get_team_standings(request, api_id):
    ...
    standings = (
        LeagueStandings.objects.filter(team__api_id=api_id)
    ...
    if not first_record or not first_record.league:
        return Response(
            {"detail": "Brak tabeli dla tej drużyny."}, status=status.HTTP_404_NOT_FOUND
        )

    first_league = first_record.league

    all_standings = (
        LeagueStandings.objects.filter(league=first_league)
        .select_related("team", "league")
...
def get_team_matches(request, api_id):
    ...
    qs = (
        LiveMatch.objects.filter(Q(home_team=team) | Q(away_team=team))
