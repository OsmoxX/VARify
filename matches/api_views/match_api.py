...
def get_live_matches(request):
    ...
    qs = (
        LiveMatch.objects.select_related("home_team", "away_team", "league")
...
def get_live_match_detail(request, match_id):
    ...
    try:
        ...
        return Response(
            {"detail": "Mecz nie znaleziony."}, status=status.HTTP_404_NOT_FOUND
        )
...
def get_match_events(request, match_id):
    ...
    events = MatchEvent.objects.filter(match_id=match_id).order_by(
        "time", "added_time", "id"
    )
...
def get_match_lineups(request, match_id):
    ...
    qs = MatchLineup.objects.filter(match_id=match_id).order_by(
        "-is_starting_xi", "shirt_number", "id"
    )
...
def get_upcoming_matches(request):
    ...
    qs = UpcomingMatch.objects.select_related(
