...
class CalendarView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ended_matches = (
            LiveMatch.objects.select_related("league", "home_team", "away_team")
            .filter(status="Ended")
        ...
        for match in ended_matches:
            ...
            country = (
                match.country_name
                or (match.league.country if match.league else None)
                or "Inne"
            )
            league_name = match.league.name if match.league else "Nieznana Liga"
            days_data.setdefault(day, defaultdict(lambda: defaultdict(list)))[country][
                league_name
            ].append(match)
        ...
        for day, countries in days_data.items():
            ...
            for country, leagues in countries.items():
                for league_name, matches in leagues.items():
                    leagues_list.append(
                        {"name": league_name, "country": country, "matches": matches}
                    )
        ...
        all_league_names = sorted(
            {league["name"] for day in structured_days for league in day["leagues"]}
        )
