...
def test_user(db):
    ...
    return User.objects.create_user(
        username="kacper", email="kacper@test.com", password="old_password123"
    )
...
def other_user(db):
    ...
    return User.objects.create_user(
        username="zajety_login", email="taken@test.com", password="password123"
    )
...
def test_change_password_wrong_current(logged_client):
    ...
    assert response.status_code == 200
    assert (
        response.context["errors"]["current_pw"] == "Obecne hasło jest nieprawidłowe."
    )
...
def test_change_password_too_short(logged_client):
    ...
    assert response.status_code == 200
    assert (
        response.context["errors"]["new_pw"]
