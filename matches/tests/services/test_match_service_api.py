        ...
        "events": [
            {
                ...
                "status": {
                    "type": "notstarted"
                },  # Ważne: tylko 'notstarted' są zapisywane!
...
def test_fetch_match_details_success(mock_get):
    ...
    mock_get.side_effect = [
        ...
        MockResponse(
            200,
            {
                "home": {
                    ...
                    "players": [
                        {
                            "player": {"name": "Szczesny", "position": "G"},
                            "substitute": False,
                        }
                    ],
                    "missingPlayers": [],
        ...
        MockResponse(
            200, {"statistics": [{"period": "ALL", "groups": [{"name": "Shots"}]}]}
        ),
    ]
        ...
        "events": [
            {
                ...
                "id": 1000,
                "tournament": {"id": 1, "name": "LaLiga"},
                "homeTeam": {"id": 1, "name": "Real"},
                "awayTeam": {"id": 5, "name": "Getafe"},
            },
            {
                "id": 1001,
                "tournament": {"id": 1, "name": "LaLiga"},
            ...
            {
                "id": 1002,
                "tournament": {"id": 7, "name": "Champions League"},
                "homeTeam": {"id": 1, "name": "Real Madrid"},
                "awayTeam": {"id": 3, "name": "Bayern Munich"},
                "status": {"description": "Ended"},
                "homeScore": {"current": 2},
                "awayScore": {"current": 2},
...
def test_fetch_last_matches_for_team_skips_corrupted_event(mock_get):
    ...
    mock_get.return_value.json.return_value = {
        "events": [
            {
                "id": 900,
                "tournament": None,  # TO WYZWOLI BŁĄD w kodzie (próba odczytania 'id' z None)
            },
            {
                "id": 901,
                "tournament": {"id": 1, "name": "LaLiga"},
