...
def test_player_creation_without_team():
    ...
    player_free_agent = Player.objects.create(api_id=99, name="Wolny Agent")
...
def test_player_api_id_unique_constraint(team_fixture):
    ...
    Player.objects.create(api_id=7, name="Cristiano Ronaldo", team=team_fixture)
    ...
    with pytest.raises(IntegrityError):
        with transaction.atomic():
            Player.objects.create(
                api_id=7, name="Inny Zawodnik z tym samym ID", team=team_fixture
            )
