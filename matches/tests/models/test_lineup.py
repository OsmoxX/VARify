...
def test_match_lineup_position_properties(match_fixture):
    # Bramkarz (Goalkeeper)
    gk = MatchLineup.objects.create(
        match=match_fixture, player_name="Szczęsny", position="G"
    )
    assert gk.position_label == "GK"
    assert gk.is_goalkeeper is True

    # Obrońca (Defender)
    def_player = MatchLineup.objects.create(
        match=match_fixture, player_name="Glik", position="D"
    )
    assert def_player.position_label == "DEF"
    assert def_player.is_goalkeeper is False

    # Pomocnik i Napastnik
    mid = MatchLineup.objects.create(
        match=match_fixture, player_name="Zieliński", position="M"
    )
    fwd = MatchLineup.objects.create(
        match=match_fixture, player_name="Lewandowski", position="F"
    )
    assert mid.position_label == "MID"
    assert fwd.position_label == "FWD"

    # Zła lub pusta pozycja (Fallback)
    unknown = MatchLineup.objects.create(
        match=match_fixture, player_name="Ktoś", position="XYZ"
    )
    empty = MatchLineup.objects.create(
        match=match_fixture, player_name="Inny", position=None
    )
...
def test_match_lineup_and_missing_str_methods(match_fixture):
    ...
    lineup_home = MatchLineup.objects.create(
        match=match_fixture, player_name="Messi", is_home_team=True
    )
    lineup_away = MatchLineup.objects.create(
        match=match_fixture, player_name="Ronaldo", is_home_team=False
    )
    ...
    missing = MissingPlayer.objects.create(
        match=match_fixture, player_name="Neymar", type="missing", is_home_team=True
    )
    doubtful = MissingPlayer.objects.create(
        match=match_fixture, player_name="Mbappe", type="doubtful", is_home_team=False
    )
...
def test_match_lineup_unique_together_constraint(match_fixture):
    ...
    MatchLineup.objects.create(
        match=match_fixture, player_name="Kamil Grosicki", is_home_team=True
    )
    ...
    MatchLineup.objects.create(
        match=match_fixture, player_name="Kamil Grosicki", is_home_team=False
    )
