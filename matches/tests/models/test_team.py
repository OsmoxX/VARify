...
def test_team_creation_and_str_method():
    ...
    team_full = Team.objects.create(
        api_id=86, name="Real Madryt", logo_url="https://example.com/real_madryt.png"
    )
...
def test_team_creation_without_logo():
    ...
    team_no_logo = Team.objects.create(api_id=101, name="Drużyna Bez Logo")
...
def test_team_api_id_unique_constraint():
    ...
    Team.objects.create(api_id=999, name="Pierwszy Zespół")
    ...
    with pytest.raises(IntegrityError):
        with transaction.atomic():
            Team.objects.create(api_id=999, name="Drugi Zespół (Klon ID)")
