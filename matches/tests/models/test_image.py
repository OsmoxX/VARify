...
def test_cached_image_creation_and_str_method():
    ...
    image = CachedImage.objects.create(
        entity_type="player", api_id=999, content=dummy_binary_data
    )
...
def test_cached_image_unique_together_constraint():
    ...
    CachedImage.objects.create(
        entity_type="team", api_id=123, content=b"pierwszy_obrazek"
    )
    ...
    with pytest.raises(IntegrityError):
        CachedImage.objects.create(
            entity_type="team", api_id=123, content=b"zupelnie_inny_obrazek"
        )
