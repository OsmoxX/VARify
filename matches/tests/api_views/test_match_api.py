...
from matches.models import League, Team, LiveMatch, MatchEvent, MatchLineup, UpcomingMatch
...
def setup_match_data():
    ...
    # Mecze Live
    lm1 = LiveMatch.objects.create(api_id=100, league=l1, home_team=t1, away_team=t2, is_top=True, status="2nd Half")
    lm2 = LiveMatch.objects.create(api_id=200, league=l2, home_team=t2, away_team=t1, is_top=False, status="1st Half")

    # Zdarzenia dla pierwszego meczu
    MatchEvent.objects.create(match=lm1, time=15, incident_type="goal", player_name="Saka")
    MatchEvent.objects.create(match=lm1, time=45, incident_type="card", player_name="Odegaard")

    # Składy dla pierwszego meczu
    MatchLineup.objects.create(match=lm1, player_name="Raya", is_home_team=True, is_starting_xi=True)
    MatchLineup.objects.create(match=lm1, player_name="Mudryk", is_home_team=False, is_starting_xi=True)

    # Mecze nadchodzące (jeden na dzisiaj, drugi na jutro)
    today = timezone.now().date()
    tomorrow = today + timedelta(days=1)
    UpcomingMatch.objects.create(api_id=300, league=l1, home_team=t1, away_team=t2, is_top=True, start_datetime=timezone.now())
    UpcomingMatch.objects.create(api_id=400, league=l2, home_team=t2, away_team=t1, is_top=False, start_datetime=timezone.now() + timedelta(days=1))
