...
def setup_leagues():
    ...
    LeagueStandings.objects.create(
        ...
        matches_played=30, matches_won=25, matches_drawn=5, matches_lost=0,
        goals_for=70, goals_against=20, goal_difference=50
    )
    LeagueStandings.objects.create(
        league=l1, team=t2, position=2, points=75,
        matches_played=30, matches_won=23, matches_drawn=6, matches_lost=1,
        goals_for=60, goals_against=25, goal_difference=35
    )
...
def test_get_league_standings_success(api_client, setup_leagues):
    ...
    assert "Arsenal" in str(results[0])  # Sprawdzamy, czy Arsenal po prostu jest w tym rzędzie tabeli
