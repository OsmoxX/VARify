...
def setup_team_data():
    ...
    t3 = Team.objects.create(api_id=30, name="Real Madrid") # Drużyna bez tabeli i meczów

    # Tabela ligowa
    LeagueStandings.objects.create(
        league=l1, team=t1, position=1, points=80,
        matches_played=0, matches_won=0, matches_drawn=0, matches_lost=0,
        goals_for=0, goals_against=0, goal_difference=0
    )
    LeagueStandings.objects.create(
        
        league=l1, team=t2, position=2, points=75,
        matches_played=0, matches_won=0, matches_drawn=0, matches_lost=0,
        goals_for=0, goals_against=0, goal_difference=0
    )
...
def test_get_team_standings(api_client, setup_team_data):
    ...
    assert "Arsenal" in str(data['results'][0])  # Używamy naszego bezpiecznego sprawdzenia stringiem
...
def test_get_team_matches_calls_api_when_under_5(mock_fetch, api_client, setup_team_data):
...
@pytest.mark.django_db
@patch('matches.api_views.team_api.fetch_last_matches_for_team')
def test_get_team_matches_api_exception_handled(mock_fetch, api_client, setup_team_data):
